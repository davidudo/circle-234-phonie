...
function verifyNumber(phone_number, MTN, GLO, NINE_MOBILE, AIRTEL) {
  ...
  const number_is_not_valid = (phone_number.length > numberLength) || (phone_number.length >= 4 && countryCode !== "+234" && phone_number[0] === "+");

  // Message display
  if (number_is_not_valid) {
    message_image.src = "icon-warning.svg";
    message_display.innerText = "The number format is invalid.";
    message_container.classList.add("red-border");
    message_container.classList.remove("hide");
  } else if (MTN.includes(network_prefix)) {
    message_container.classList.remove("green-border", "yellow-border", "red-border");
...
function autoComplete(phone_number, num_prefix_store) {
  ...
  ul.addEventListener('click', (e) => { selectSuggestion(e, phone_number) });
  ...
  function autoComplete(phone_number, num_prefix_store) {
    return num_prefix_store.filter(
      (value) => value.toLowerCase().includes(phone_number.toLowerCase())
    );
  };
